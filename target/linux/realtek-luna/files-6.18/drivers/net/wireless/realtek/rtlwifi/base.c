...
static void _rtl_init_mac80211(struct ieee80211_hw *hw)
{
	...
	/* Advertise AP-mode U-APSD. Without it, cfg80211 (nl80211.c "ignore WME
	 * attributes if iface/sta is not capable") strips the per-STA U-APSD
	 * qosinfo hostapd passes down, so sta->uapsd_queues stays 0 and mac80211
	 * never releases buffered downstream frames on a power-saving client's
	 * QoS-Null triggers -- deadlocking every PS client's traffic (the DHCP
	 * OFFER never reaches the phone; bulk data stalls). mac80211/cfg80211
	 * never set this flag themselves; the driver must. Fixes all rtlwifi
	 * PCIe APs serving a power-saving (uAPSD) station. */
